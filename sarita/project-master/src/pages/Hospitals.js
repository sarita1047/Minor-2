...
const Hospital = () => {
  ...
  const [filteredHospital, setFilteredHospital] = useState(hospitalList);
  const [searchHospital, setSearchHospital] = useState('');

  const dataToUse = searchHospital ? filteredHospital : hospitalList;
  ...
  return (
    <section className="d-flex justify-space-between">
      <div className="col col-3">
        <img src={require("../assets/images/hsptl.png")} alt="dp" style={{ width: "100%", marginLeft: -50 }} />
      </div>
      ...
      <div className="col col-9 mt-4 bg-white p-4">
        ...
        >
          ...
          <button
            className={`col col-2 btn btn-${showHospitalList ? "success" : "danger"
              } btn-sm m-0`}
            style={{ height: 40 }}
            onClick={(_) => setShowHospitalList(!showHospitalList)}
          >
            {showHospitalList ? "Add Hospital" : "Back"}
          </button>
        </div>
        {showHospitalList ? (
          hospitalList?.length ? (
